...
public class CustomerMapper {
    ...
    public static int numberOfCustomers () throws LoginSampleException {
        ...
        try{
            Connection con = Connector.connection();
            String sql="SELECT Count(users.email) AS numberOfCustomers \n" +
                    "FROM orders where not email='admin'";
            PreparedStatement ps = con.prepareStatement(sql);
            ResultSet resultSet = ps.executeQuery();
            while (resultSet.next()){
    ...
    public static Customer[] listOfCustomersWithOrders() throws LoginSampleException {
        ...
        try{
            Connection con = Connector.connection();
            String customersSQL = "select users.email, firstname, lastname, saldo from users left join " +
                    "orders on users.email=orders.email where not users.email='admin' group by email;\n";
